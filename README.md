...
Приложение построено на паттерне MVC(S) с разделением на модели (Players, Matches), контроллеры для обработки запросов и сервисы для логики подсчёта очков. Данные хранятся в MySQL через SQLAlchemy, миграции управляются Alembic. Фронтенд — ручная верстка на HTML/CSS без библиотек. Реализована пагинация и фильтрация матчей.
